...
export default async function ProductosPage({ params, searchParams }: Props) {
  ...
  const totalProductos = tienda._count?.Producto || 0;
  const totalPages = Math.ceil(totalProductos / pageSize);
  const stockTotal = tienda.Producto.reduce((sum, p) => sum + (p.stock || 0), 0);

  // Filtros únicos
  const categorias = [...new Set(tienda.Producto.map(p => p.Categoria.nombre))].sort();
  const tallas = [...new Set(tienda.Producto.map(p => p.talla).filter(Boolean))].sort();
  const generos = [...new Set(tienda.Producto.map(p => p.tipo_genero).filter(Boolean))].sort();
